...
AiPlus Econ Agent Team 专门解决**应用经济学研究的角色分工与执行**问题。其它 AiPlus 插件解决相邻但不同的问题：

| 插件 | 解决的痛 | 为什么不是 Econ Agent Team |
|---|---|---|
...
Econ Agent Team 和 [aiplus-agent-team](https://github.com/izhiwen/aiplus-agent-team) 是兄弟模块 —— 可以共存在同一个项目里（比如同时维护 paper 和 replication package 软件仓库的研究者）。

## 解决方案

**用永久研究团队替代单 agent 漂移。**

AiPlus Econ Agent Team 在你的项目中安装一个由 8 个核心角色组成的永久虚拟团队：Advisor（导师顾问）、PI（你 / 主作者）、Theorist（理论建模）、PM（项目管理）、RA-Stata（Stata 助研）、RA-Python（Python 助研）、Referee（内部 referee）、Replicator（复现工程师）。每个角色有自己的 persona、workspace、memory namespace。Owner（你，主作者）只和 Advisor 与 PI 对话；PI 协调其它角色。
...
- **专家目录** —— 11 位专家（文献综述员、写作师、计量专家、复现工程师、史料专家、Job Talk Coach 等）默认休眠，PI 在 trigger 命中时召唤。
...
Econ Agent Team 是协调层。建在四个已有 AiPlus 插件之上：
...
2. **专家目录** —— 11 个 on-demand 专家，trigger 命中才召唤。
...
- `core/templates/` —— 8 个核心角色的 TOML 配置和 markdown persona，加 team-wide `econ-team.toml`
- `core/templates/personas/` —— 角色 persona prompts (advisor, pi, theorist, pm, ra-stata, ra-python, referee, replicator)
- `core/templates/experts/` —— 专家角色配置
- `core/docs/` —— 设计原理、路由协议、memory 模型、worktree 策略、安全边界
...
1. **先开 issue** —— 比 typo 大的改动都先开 issue。`aiplus-econ-agent-team` 范围紧。
...
5. **验收标准强制** —— 见 `.aiplus/econ-agent-team/acceptance/v0.1.0/schema.yaml`。行为变化必须更新 schema 和配套 `.test.sh`。

## 安全边界

AiPlus Econ Agent Team 不会：
